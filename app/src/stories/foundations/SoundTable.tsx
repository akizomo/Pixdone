...
export function SoundTable() {
  ...
  return (
    <div>
      ...
      >
        <strong style={{ color: 'var(--pd-color-text-secondary)' }}>現在のビジュアルテーマ:</strong>{' '}
        {vt.icon} {vt.name} — サウンドパック <code style={{ fontFamily: 'var(--pd-font-mono)', fontSize: 12 }}>{vt.soundPackKey}</code>
        （<code style={{ fontFamily: 'var(--pd-font-mono)', fontSize: 12 }}>ThemeProvider</code> → エンジンで音色が切り替わります）
      </div>
        ...
        <tbody>
          {soundKeys.map((key) => {
            ...
            return (
              <tr key={key}>
                <td style={tdStyle}>
                  <code style={{ fontFamily: 'var(--pd-font-mono)', fontSize: '0.8125rem' }}>{token.key}</code>
                </td>
