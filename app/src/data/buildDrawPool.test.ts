...
describe('buildDrawPool — challenge deadline behavior (fighter_punch)', () => {
  beforeEach(() => vi.useFakeTimers());
  afterEach(() => vi.useRealTimers());

  const hasPunch = (pool: { key: string }[]) => pool.some(e => e.key === 'fighter_punch');

  it('within deadline + owned → included', () => {
    vi.setSystemTime(WITHIN);
    const pool = buildDrawPool(false, 'arcade', ['fighter_punch'], ['fighter_punch']);
    expect(hasPunch(pool)).toBe(true);
  });

  it('within deadline + not owned + free → excluded', () => {
    vi.setSystemTime(WITHIN);
    const pool = buildDrawPool(false, 'arcade', ['fighter_punch'], []);
    expect(hasPunch(pool)).toBe(false);
  });

  it('within deadline + not owned + premium → excluded (still active challenge)', () => {
    vi.setSystemTime(WITHIN);
    const pool = buildDrawPool(true, 'arcade', ['fighter_punch'], []);
    expect(hasPunch(pool)).toBe(false);
  });
