...
try {
  ...
  const hints: Record<string, string> = {
    ENV_CONFIG:
      '必須の環境変数が Vercel に設定されていません。Production に DATABASE_URL と SESSION_SECRET を追加してください。docs/DEPLOYMENT_CONTRACT.md 参照。',
    DB_CONNECT:
      'Postgres に接続できません。Vercel の DATABASE_URL（Supabase の Pooler 可）、SSL、パスワードの URL エンコードを確認してください。docs/SUPABASE_DB_SETUP.md 参照。',
    AUTH_SETUP:
      'セッション初期化に失敗しました。SESSION_SECRET を設定し、DATABASE_URL が connect-pg-simple（sessions テーブル）からも使えることを確認してください。',
    UNKNOWN:
      'API の起動に失敗しました。Vercel のログ（registerRoutes failed at startup）を確認してください。',
  };
  app.use((_req, res) => {
    res.status(500).json({
