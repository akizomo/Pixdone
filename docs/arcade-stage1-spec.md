...
[壁]       y=0  〜 y=52   (上 52px ≈ 43%)
[幅木]     y=52 〜 y=60   (8px のトリムライン)
[床]       y=60 〜 y=120  (下 60px = 50%)

[Bit キャラクター]
  位置: x=W/2 (中央), 足元 y=60 (床面にぴったり立つ)
  サイズ: 8×8 論理px × 4倍拡大 = 32×32 物理px
  ポーズ: neutral (まっすぐ立つ、両手下げ)

[フロアライン]
  y=60 に 2px の水平線（--pd-arcade-world-trim 色）
```

> 8×8 ベースで 4倍拡大（1ドット = 4×4px）。
> Pixel Vitalism の「制約の中の本質」に合致したサイズ。
> キャラクター頭頂が y=28 になり、壁エリア（y=52）より上には出ない = 部屋の中に収まる。

### Bit キャラクター描画（Stage 1 Neutral ポーズ）

```
8×8 グリッド（1論理px = canvas上では 4×4 物理px = 32px 合計）

 .BBBB.    行0: 頭部上
 BBBBBB    行1: 頭部
 B.EE.B    行2: 顔（E=eye の左右1px）
 BBBBBB    行3: 頭部下
 .BBBB.    行4: 胴体上（首あたり）
 .BBBB.    行5: 胴体下
 B.BB.B    行6: 腰〜脚
